...
public class DayView extends CalendarView {
	...
	public DayView(DayViewConfig desc, Component upperLeftCornerComponent) throws Exception {
		...
		rowHeader = new TimeLabelPanel(desc, new TimeOfDay(this.config.getDayStartHour(), 0), new TimeOfDay(this.config.getDayEndHour(), 0));
		/* ------------------------------------------------------- */
	...
	public void refresh0() throws Exception {
		...
		rowHeader.setStartEnd(new TimeOfDay(this.config.getDayStartHour(), 0), new TimeOfDay(this.config.getDayEndHour(), 0));
		rowHeader.setFooterHeight(getFooterHeight());
	...
	private void createColumns() throws Exception {
		...
		for (int it = 0; it < cols; it++) {
			...
			bizcal.common.Calendar cal = ( bizcal.common.Calendar ) getSelectedCalendars().get(iCal);
			...
			if (config.isShowDateFooter()) {
				JLabel footer = new JLabel(broker.getDateFooter(cal.getId(), interval2.getStartDate(), colEvents));
	...
	private Object getCalendarId(int colNo) throws Exception {
		...
		if (dayCount != 0)
			pos = colNo / dayCount;
		bizcal.common.Calendar cal = ( bizcal.common.Calendar ) getSelectedCalendars().get(pos);
		return cal.getId();
	}
	 ...
	 * 
	 * @version <br>
	 *          $Log: DayView.java,v $ Revision 1.38 2008/12/12 16:20:11 heine_
	 *          *** empty log message ***
	 * 
