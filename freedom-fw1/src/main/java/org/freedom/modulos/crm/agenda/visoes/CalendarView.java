 ...
 * 
 * @version <br>
 *          $Log: CalendarView.java,v $ Revision 1.37 2008/10/30 10:42:52 heine_
 *          *** empty log message ***
 * 
...
public abstract class CalendarView {
	...
	private class FrameAreaMouseListener extends MouseAdapter implements MouseMotionListener {
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (_event.isBackground()) {
				MouseEvent me = new MouseEvent(calPanel, e.getID(), e.getWhen(), e.getModifiers(), e.getX() + _frameArea.getX(), e.getY() + _frameArea.getY(), e.getClickCount(), e.isPopupTrigger(), e
						.getButton());
		...
		private void moveDrag(FrameArea baseFrameArea, MouseEvent e) {
			...
			if (e.getPoint().x < 0) {
				...
				if (e.getPoint().x > currWidth)
					if (e.getPoint().x > currWidth + gap2right) {
						...
						// -------------------------------------------------------
						// */
						newXPos = findNextGreaterVerticalLine(baseFrameArea.getX());
						// greater
					}
				/* ------------------------------------------------------- */
			}
				...
				if (Math.abs(diffPoint) > getTimeSlotHeight() || _shiftKey) {
					...
					if (baseFrameArea.getY() + mov >= calPanel.getY()) {
						...
						if (baseFrameArea.getChildren() != null && baseFrameArea.getChildren().size() > 0) {
							...
							// the panels bottom
							baseFrameArea.setBounds(baseFrameArea.getX(), baseFrameArea.getY(), baseFrameArea.getWidth(), calPanel.getHeight() - baseFrameArea.getY());
							/*
							 * --------------------------------------------------
							 * -----
							 */
			...
			try {
				_frameArea.setMovingTimeString(getDate(_frameArea.getX(), _frameArea.getY()), getDate(_frameArea.getX(), _frameArea.getY() + _frameArea.getHeight()));
		...
		private void resizeDrag(FrameArea baseFrameArea, MouseEvent e) {
			...
			// time slot height!
			/* ------------------------------------------------------- */
			if (( currentArea.getHeight() + currentPoint.y ) < CalendarView.this.desc.getMinimumTimeSlotHeight() || ( currentArea.getHeight() + currentPoint.y ) < getTimeSlotHeight()) {
				/* ------------------------------------------------------- */
				return;
				/* ------------------------------------------------------- */
			}
				...
				if (gap2leftColumn + currentPoint.x < 0) {
					...
					if (baseFrameArea.getChildren() != null) {
						...
						for (FrameArea fa : baseFrameArea.getChildren()) {
							 ...
							 * -----
							 */
							if (fa.getX() > findNextSmallerVerticalLine(currX + currentPoint.x)) {
								// remove
					...
					// else
					{
						...
						if (crossPoint > currWidth + gap2rightColumn) {
							...
							newLines = findUndrawnLines(currX + this.mouseXold, currX + currentPoint.x, baseFrameArea.getX());
							/*
							 * --------------------------------------------------
							 * -----
							 */
							if (newLines != null && newLines.size() > 0) {
								 ...
								 * ---------
								 */
								currentArea.setBounds(currentArea.getX(), 0, currWidth, calPanel.getMaximumSize().height);
								for (Integer i : newLines) {
									...
									if (additionalFrames.containsKey(i)) {
										...
										// + i);
										afa.setVisible(true);
										afa.setBounds(afa.getX(), 0, currWidth, calPanel.getMaximumSize().height);
									...
									fa.setBounds(i, 0, currWidth, findNextGreaterHorizontalLinePos(currY + currentPoint.y));
									fa.setEvent(_event);
									...
									if (lastCreatedFrameArea != null) {
										 ...
										 * -----------------
										 */
										lastCreatedFrameArea.setBounds(lastCreatedFrameArea.getX(), 0, currWidth, calPanel.getMaximumSize().height);
		...
		public void mouseReleased(MouseEvent e) {
			...
			try {
				...
				if (additionalFrames != null && additionalFrames.size() > 0) {
					...
					date2 = getDate(lastArea.getX() + 2, lastArea.getY() + lastArea.getHeight());
					/* ------------------------------------------------------- */
				}
				else {
					/* ------------------------------------------------------- */
					...
					/* ------------------------------------------------------- */
					date2 = getDate(_dragArea.getX() + 10, _dragArea.getY() + _dragArea.getHeight());
					/* ------------------------------------------------------- */
				}
				if (_lasso) {
					/* ------------------------------------------------------- */
				...
				if (pX >= currPos) {
					...
					if (!( pX > currPos && pX < ( currPos + gap ) )) {
						...
						if (_lasso) {
							...
							List<Integer> newLines = findUndrawnLines(this.mouseXold, e.getPoint().x, currPos);
							...
							_dragArea.setBounds(dx, dy, dw, findNextGreaterHorizontalLinePos(e.getPoint().y) - dy);
						 ...
						 */
						{
							...
							// smaller
							// than the boundX
							if (lastCreatedFrameArea != null && pX <= lastCreatedFrameArea.getX()) {
								/*
								 * ----------------------------------------------
								 * ---------
								 */
							...
							if (lastCreatedKey == null)
								newLines = findUndrawnLines(this.mouseXold, e.getPoint().x, currPos);
							else
								newLines = findUndrawnLines(this.mouseXold, e.getPoint().x, lastCreatedKey);
							/*
							 * --------------------------------------------------
							 * -----
							 */
							if (newLines != null && newLines.size() > 0) {
								...
								for (Integer i : newLines) {
									...
									if (lastCreatedFrameArea != null) {
										 ...
										 * -----------------
										 */
										lastCreatedFrameArea.setBounds(lastCreatedFrameArea.getX(), 0, gap, calPanel.getMaximumSize().height);
									}
									this.lastCreatedFrameArea = fa;
									this.lastCreatedKey = i;
									/*
							 ...
							 */
							try {
								lastCreatedFrameArea.setBounds(lastCreatedFrameArea.getX(), 0, gap, findNextGreaterHorizontalLinePos(e.getPoint().y));
								/*
								 * ----------------------------------------------
								 * ---------
								 */
								lastCreatedFrameArea.setEndTime(getDate(currPos, lastCreatedFrameArea.getY() + lastCreatedFrameArea.getHeight()));
							}
							catch (Exception e2) {
							}
						}
						...
						if (_dragArea instanceof FrameArea)
							( ( FrameArea ) _dragArea ).setEndTime(getDate(currPos, _dragArea.getY() + _dragArea.getHeight()));
	...
	protected FrameArea getFrameArea(Object calId, Event event) {
		return _frameAreaMap.get("" + calId + event.getId() + event.getStart().getTime());
	}
