	...
	public void setVisible( boolean show ) {
		...
		if ( AplicativoPDV.bECFTerm ) {
			...
			if ( show ) {
				...
				int result = AplicativoPDV.abreCaixa( con, ecf );
				...
				if ( result == -1 ) {
					...
					if ( AplicativoPDV.caixaAberto( con ) || AplicativoPDV.pegaValorINI( con ) ) {
						iniVenda();
						if ( obrigaOrcamento ) {
