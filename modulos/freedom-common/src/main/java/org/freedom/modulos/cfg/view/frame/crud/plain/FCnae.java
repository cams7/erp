...
public class FCnae extends FDados implements ActionListener {
	...
	private void imprimir(TYPE_PRINT bVisualizar) {
		...
		/*
		 * PreparedStatement ps = null; ResultSet rs = null; ImprimeOS imp = new
		 * ImprimeOS( "", con ); int linPag = imp.verifLinPag() - 1;
		 * 
		 * try {
		 * 
		 * imp.limpaPags(); imp.montaCab(); imp.setTitulo(
		 * "Relatório de atividades CNAE" );
		 * 
		 * String sSQL =
		 * "SELECT CODCNAE, DESCCNAE, SIGLAPAIS,DDIPAIS FROM SGPAIS ORDER BY DescCnae"
		 * ;
		 * 
		 ...
		 * 
		 * imp.say( 0, imp.normal() ); imp.say( 2, "Cód.pais" ); imp.say( 15,
		 * "Nome" ); imp.say( 80, "Sigla" ); imp.say( 100, "DDI" );
		 * 
		 * imp.pulaLinha( 1, imp.normal() ); imp.say( 0,
		 * StringFunctions.replicate( "-", 135 ) );
		 * 
		 * }
		 * 
		 * imp.pulaLinha( 1, imp.normal() ); imp.say( 2, rs.getString( "CodCnae"
		 * ) != null ? rs.getString( "CodCnae" ) : "" ); imp.say( 15,
		 * rs.getString( "DescCnae" ) != null ? rs.getString( "DescCnae" ) : ""
		 * ); imp.say( 80, rs.getString( "SiglaPais" ) != null ? rs.getString(
		 * "SiglaPais" ) : "" ); imp.say( 100, rs.getString( "DDIPais" ) != null
		 * ? rs.getString( "DDIPais" ) : "" );
		 * 
		 * 
		 * }
		 * 
		 * imp.pulaLinha( 1, imp.normal() ); imp.say( imp.pRow() + 0, 0,
		 * StringFunctions.replicate( "=", 135 ) );
		 * 
		 ...
		 * Funcoes.mensagemErro( this, "Erro consulta tabela de paises!" +
		 * err.getMessage(), true, con, err ); }
		 * 
		 * if ( bVisualizar==TYPE_PRINT.VIEW ) { imp.preview( this ); } else {
		 * imp.print(); }
		 */
	}
