...
public class FAcesso extends FFDialogo implements ArvoreFace, CarregaListener, JComboBoxListener {
	...
	public void montaArvore() {
		...
		try {
			...
			String sSQL = "SELECT (SELECT AC.TPACESSOMU FROM SGACESSOMU AC WHERE AC.CODEMP = ? "
					...
					+ "WHERE SIS.CODSIS=MN.CODSIS AND MO.CODSIS=MN.CODSIS AND " + "MN.CODMENU!=0 AND "
		...
		try {
			while (true) {
				...
				if ((iSubMenuAnt != -1) && (iSubMenu == 0)) {
					...
					item = new DefaultMutableTreeNode(
							Funcoes.copy(rs.getString("DescMenu"), 0, 40) + "[" + (iConta++) + "]");
					pai.add(item);
					adicAcesso((rs.getString(1) == null ? 'A' : rs.getString(1).toCharArray()[0]), rs.getInt("CodMenu"),
							rs.getInt("CodSis"), rs.getInt("CodModu"), iNivel);
				} else {
	...
	public ImageIcon getImagem(int iLinha, boolean bNoh, Object src) {
		...
		if (iInd == -1)
			return null;
		return Icone.novo(
				"ba" + ((iInd < vAcessos.size()) && (iInd != -1) ? vAcessos.elementAt(iInd)[0] : "0000") + ".gif");
	}
	...
	private void copiarPermissoes(String usuario) throws SQLException {
		...
		// Deleta todas as permissões do usuário para poder inserir suas novas
		// permissões, de acordo com o usuário que foi selecionado para copia.
		...
		// Realiza um select/insert das permissões do usuário selecionado para o
		// usuário que receberá as novas permissões.
		StringBuilder sql = new StringBuilder();
		sql.append(
				"insert into sgacessomu (codemp, codfilial, idusu, codsis, codmenu, codmodu, tpacessomu, planomu ) ");
